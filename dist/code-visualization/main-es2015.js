...
let SortingFormComponent = class SortingFormComponent {
    constructor(bubbleSortService, quickSortService) {
    ...
    callBubbleSort(nodes) {
        const array = nodes;
        let algorithm = document.getElementById("algorithm-selection").value;
        if (algorithm == "1") {
    ...
    getSelectedAlgorithm(event) {
        let algorithm = event.target['options'][event.target['options'].selectedIndex].text;
...
SortingFormComponent.ctorParameters = () => [
...
let BubbleSortService = class BubbleSortService {
    ...
    run(array, end, i, j) {
        ...
        setTimeout(() => {
            if (Number(array[j].value) > Number(array[j + 1].value)) {
                this.selector.selectNode(array[j + 1], _Constant__WEBPACK_IMPORTED_MODULE_3__["Constant"].SELECTED_COMPARISON);
            }
        }, 500);
        setTimeout(() => {
            if (i < array.length - 1 && j < end) {
                ...
                array[j + 1].selected = '';
                if (j + 1 == end) {
                    this.selector.selectNode(array[j + 1], _Constant__WEBPACK_IMPORTED_MODULE_3__["Constant"].SORTED_CLASS);
                    end--;
                    if (end <= 1) {
                        this.selector.selectNode(array[0], _Constant__WEBPACK_IMPORTED_MODULE_3__["Constant"].SORTED_CLASS);
                    }
                }
            }
            if (j + 1 < end) {
...
BubbleSortService.ctorParameters = () => [
...
// 2,6,5,3,8,7,1,0
let QuicksortService = class QuicksortService {
    constructor(selector) {
        this.selector = selector;
        this.frontEndArray = [];
        this.nodes = [];
    ...
    createState() {
        ...
        const state = new _QuickSortState__WEBPACK_IMPORTED_MODULE_2__["QuickSortState"](array);
        this.saveState(state);
    }
    saveState(state) {
        this.states.push(state);
    }
    getQuickSortNodes(array) {
        const nodes = [];
    ...
    organizePositions(nodes, pivot, clean) {
        let less = [];
        let greater = [];
        for (const node of nodes) {
            if (node !== pivot) {
    ...
    fillPivot(pivot, newArray) {
        pivot.sorted = true;
        pivot.isPivot = false;
        newArray.push(pivot);
    }
    fillArray(arrayToFill, oldArray) {
        ...
        setTimeout(() => {
            if (this.states.length > 0) {
                ...
                for (let i = 0; i < nodes.length; i++) {
                    ...
                    if (nodes[i].isPivot) {
                        this.selector.selectNode(nodeToSelect, _Constant__WEBPACK_IMPORTED_MODULE_5__["Constant"].PIVOT_CLASS);
                    }
                    else if (nodes[i].position == _Constant__WEBPACK_IMPORTED_MODULE_5__["Constant"].LESS) {
                        this.selector.selectNode(nodeToSelect, _Constant__WEBPACK_IMPORTED_MODULE_5__["Constant"].LESS_CLASS);
                    }
                    else if (nodes[i].position == _Constant__WEBPACK_IMPORTED_MODULE_5__["Constant"].GREATER) {
                        this.selector.selectNode(nodeToSelect, _Constant__WEBPACK_IMPORTED_MODULE_5__["Constant"].GREATER_CLASS);
                    }
                    else if (nodes[i].sorted == true) {
                        this.selector.selectNode(nodeToSelect, _Constant__WEBPACK_IMPORTED_MODULE_5__["Constant"].SORTED_CLASS);
                    }
                }
                this.presentQuickSortStates();
            }
...
QuicksortService.ctorParameters = () => [
